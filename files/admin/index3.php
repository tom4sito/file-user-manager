...
function listFolderFiles($dir){
    ...
    foreach($folder_files as $ff){
    	if(is_dir($dir.'/'.$ff)){
    		...
    		echo "<li>";
    		echo "<i class='fas fa-folder'></i><span class='directory' data-toggle='collapse' href='#{$ff}' aria-expanded='false' aria-controls='{$ff}' > ";
    		...
    		echo "<li class='file'><i class='far fa-file'></i> {$ff} <input type='checkbox' name='files[]' value='$dir_path' > ";
    	}
			...
			<div class="row">
				<div class="col-md-auto root-tree-panel">
					...
					<form action="delete_assign_file.php" method="post">
						...
						<span class='directory'> Root Directory
						<i class='fas fa-caret-down'></i> 
							<i class='fas fa-folder-plus' path='' data-toggle='modal' data-target='#createDirModal' title='click to create new folder in root directory'></i>
							<i class='fas fa-file-upload' path='/' data-toggle="modal" data-target="#uploadModal" title="click here to upload file to root folder"></i>
							<input type='text' class='display dir-name'>
							<span  class='create-dir-btn-root display' path="var-www-html-uploads">Create Directory</span>
						</span>
...
<script type="text/javascript">
	$( document ).ready(function() {
		...
		//root directory create folder button click
		// $("span.create-dir-btn-root").on('mousedown', function(event) {
		// 	event.preventDefault();
		// 	console.log("mousedown on root");
		// }).on('click', function(){
		// 	var dir_name = $(this).prev().val();
		// 	console.log(dir_name);
		// 	ajaxCreateDir(dir_name);
		// });

		$("i").click(function(){
			var class_name = this.className;
			if(class_name.includes("fa-folder-plus")){
				var dir_path = $(this).attr('path');
				console.log(dir_path);
				$("#createDirBtn").attr("path", dir_path);
			}
		});
		...
		// deletes directory --------------------------------------------------
		$("i").click(function(){
			if(this.className.includes("fa-folder-minus")){
				var dir_path = $(this).attr('path');
				if(confirm("Are you sure you want to delete this folder?")){
					ajaxDeleteDir(dir_path);
				}
		...
		$("i").click(function(){
			if(this.className.includes("fa-file-upload")){
				$("#upload-path-id").text($(this).attr("path"));
				$("#upload-path").val($(this).attr("path"));		
			}
		});
